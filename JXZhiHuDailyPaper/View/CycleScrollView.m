...
@property (nonatomic, strong) JXStorieModel *storyModel;
...
- (instancetype)initWithFrame:(CGRect)frame
                   storyModel:(JXStorieModel *)storyModel{
...
+ (instancetype)attchToView:(UIView *)view
    ...
    TopView *topView = [[[NSBundle mainBundle] loadNibNamed:@"TopView"
                                                    ...
                                                    options:nil]
                        lastObject];
    topView.frame = CGRectMake(0, -45, SCREEN_WIDTH, 265);
    topView.observe = YES;
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (offSetY<=0&&offSetY>=-90) {
        self.frame = CGRectMake(0, -45 - 0.5 * offSetY, SCREEN_WIDTH, 265 - 0.5 * offSetY);
    }else if(offSetY<-90){
...
- (void)setDetailStory:(JXContentModel *)detailStory{
...
- (void)setStoryModel:(JXStorieModel *)storyModel{
    _storyModel = storyModel;
    self.titleLabel.text = storyModel.title;
    [self.imageView sd_setImageWithURL:[NSURL URLWithString:[storyModel.images firstObject]] placeholderImage:[UIImage imageNamed:@"Field_Mask_Bg"]];
}
